package ru.arslanov.flash.utils {
	...
	public class Logger {
		...
		static public function init( stage:Stage, position:String = "right" ):void {
			_stage = stage;
			_position = position;
			...
			_stage.addChild( _btnEdit );
			_stage.addChild( _btnSide );
			...
			_stage.addEventListener( Event.RESIZE, hrStageResize );
			_stage.addEventListener( MouseEvent.MIDDLE_CLICK, hrMiddleClick );
			_stage.addEventListener( TouchEvent.TOUCH_END, hrTouchEnd );
			_stage.addEventListener( TouchEvent.TOUCH_TAP, hrTouchTap );
			
			_keyCtrl = new KeyController( _stage );
		...
		static private function updateSize():void {
			var ww:uint = _stage.stageWidth;
			var hh:uint = _stage.stageHeight;
			var xx:uint = 0;
			var yy:uint = 0;
			
			switch ( _position ) {
				case POSITION_BOTTOM:
					ww = _stage.stageWidth;
					hh = int( _stage.stageHeight / 2 );
					yy = hh;
				break;
				case POSITION_LEFT:
					ww = int( _stage.stageWidth / 2 );
					hh = _stage.stageHeight;
				break;
				case POSITION_RIGHT:
					ww = int( _stage.stageWidth / 2 );
					hh = _stage.stageHeight;
		...
		static private function toTop():void {
			_stage.setChildIndex( _body, _stage.numChildren - 1 );
			_stage.setChildIndex( _btnEdit, _stage.numChildren - 1 );
			_stage.setChildIndex( _btnSide, _stage.numChildren - 1 );
		}
	}
